...
| **长段折叠**：每个分支都一样的开头、以及任何一条一路上不分叉的长段，隐藏轮数达到阈值就折成一个节点，点它即展开（工具栏也能手动折叠 / 展开；阈值按每一段各算） | 已完成 |
| **主对话的换位规则**（无需开关）：点树里「已被收进 Tree」的节点 → 把它放回主对话、同时把你在看的那条收起；点「已在主对话」的节点 → 只切过去，不收不放；点当前版本自己的节点 → 只回到「对话」分页 | 已完成 |
| 「只看最近 N 轮」的截断视图 | 待定 |
...
这三家界面的差别在于 **操作按钮放在哪里、有哪些**，因此预设只改变这一点，颜色始终沿用 DSH 原生配色。在 **设置 → TreeView** 中选择，面板内有实时预览。
