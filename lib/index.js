...
import { normalizeLabel, readState, setDemoted, setLabel, stateFilePath } from './tree-state.js';
...
async function sessionParsedData(ctx, record) {
  ...
  const entry = { key, live: live === undefined ? undefined : new WeakRef(live), turns, effect, time, inheritedTurns };
...
async function tree(ctx, sessionId) {
  ...
  const versions = flat.map(({ entry, depth }, index) => {
    ...
    const isFork = effect === undefined && inheritedTurns > 0;
    return {
      sessionId: entry.id,
      ...
      // A fork that never grew a turn of its own is a copy of the history. It is
      // not drawn by default, but the panel still needs to know what it is when
      // the user has explicitly collected it into the tree.
      ...isFork && ownTurns.length === 0 ? { copy: true } : {},
      turns,
      // Only used to drop copies; never sent.
      __ownTurnCount: ownTurns.length,
    };
  });
  // A copy with nothing of its own is not a branch, it is a photocopy: drawing
  // it would put a second full-length chain on the canvas and make the real
  // history look like it forked twice — which is exactly the phantom branch a
  // user reported. Three things keep a copy anyway:
  //
  //   * the user collected it into the tree ("收到 Tree 里"), which is an
  //     explicit statement that it belongs here,
  //   * the root, which is the trunk and never a copy, and
  //   * deleted versions, which the panel draws as ghosts.
  //
  // A fork that kept talking after the copy always stays: it has new turns.
  const branches = versions.filter((version) => version.__ownTurnCount !== 0
    || version.parentSessionId === undefined
    || version.deleted === true
    || state.demoted[version.sessionId] === true);
  for (const version of branches) delete version.__ownTurnCount;
  // An archived fork is not necessarily a branch of this conversation.
  // dsh-sidenote, for one, forks the session for a side chat and archives the
  // fork so the sidebar stays clean; drawing that here would show a "branch"
  // the user never made. So an archived version is shown only when it is still
  // part of this tree: either the tree itself put it away (the user demoted it,
  // which is exactly "keep it in the tree, out of the sidebar"), or a surviving
  // branch descends from it, which keeps chains anchored.
  const childCount = new Map();
  for (const version of branches) {
    if (version.parentSessionId === undefined) continue;
    childCount.set(version.parentSessionId, (childCount.get(version.parentSessionId) ?? 0) + 1);
  }
  const visible = branches.filter((version) => version.archived !== true
    || state.demoted[version.sessionId] === true
    || (childCount.get(version.sessionId) ?? 0) > 0);
  return { sessionId, versions: visible };
}
 ...
 *
 * This is the supported direction, so it calls the registry's own
 * `archiveSession` instead of writing state by hand. The membership is recorded
 * in our sidecar as well, because archiving is shared with other plugins: the
 * tree has to know this one was hidden *by the tree* and must keep drawing it.
 */
async function demoteVersion(ctx, sessionId) {
  const registry = ctx.get('workspaceRegistry');
  if (registry === undefined) throw new Error('workspaceRegistry 不可用，无法收起该分支。');
  if (typeof registry.archiveSession !== 'function') throw new Error('此 dsh 版本未提供归档接口。');
  const already = Array.isArray(registry.archivedSessionIds) && registry.archivedSessionIds.includes(sessionId);
  if (!already) await registry.archiveSession(sessionId);
  setDemoted(stateFilePath(), sessionId, true);
  return { ok: true, sessionId, inMainChat: false, changed: !already };
}
