...
import { labelsFilePath, normalizeLabel, readLabels, setLabel } from './labels.js';
...
async function tree(ctx, sessionId) {
  ...
  // Node labels are a fork feature: they live in a sidecar, never in the log
  // (see lib/labels.js for why), so the tree payload simply carries them along.
  const labels = readLabels(labelsFilePath());
  ...
  const versions = flat.map(({ entry, depth }, index) => {
    ...
    return {
      ...
      ...labels[entry.id] === undefined ? {} : { label: labels[entry.id] },
      ...effect === undefined ? {} : {
  ...
  // An archived fork is not a branch of this conversation. dsh-sidenote, for
  // one, forks the session for a side chat and archives the fork so the sidebar
  // stays clean; drawing it here would show a "branch" the user never made. An
  // archived version is still kept when something descends from it, so a
  // surviving chain never loses its anchor.
  ...
  const visible = versions.filter((version) => version.archived !== true || (childCount.get(version.sessionId) ?? 0) > 0);
...
function labelVersion(body) {
  ...
  setLabel(labelsFilePath(), sessionId, label);
