...
function archivedSessionIdSet(ctx) {  try {
...
async function tree(ctx, sessionId) {
  ...
  // part of this tree: either the tree itself put it away (the user collected
  // it, which is exactly "keep it in the tree, out of the sidebar"), or a
  // surviving branch descends from it, which keeps chains anchored.
  ...
  const visible = versions.filter((version) => version.archived !== true
...
async function collectOthers(ctx, body) {
  ...
  for (const id of targets) {
    try {
      const already = Array.isArray(registry?.archivedSessionIds) && registry.archivedSessionIds.includes(id);
      if (!already && typeof registry?.archiveSession === 'function') await registry.archiveSession(id);
      setDemoted(stateFilePath(), id, true);
      collected.push(id);
    } catch (error) {
      // A version we cannot hide stays visible; the rest still get collected.
    }
  }
  return { ok: true, collected, stopped };
}
...
async function handleRoute(ctx, request, response) {
  try {
    ...
    // `busy` rides along so the panel can ask about stopping those turns.
